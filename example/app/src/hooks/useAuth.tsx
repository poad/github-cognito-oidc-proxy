...
const useAuth = () => {
  ...
  const location = typeof window !== 'undefined' ? new URL(window.location.href) : undefined;
  const params = location?.searchParams;
  const code = params?.has('code') ? params.get('code') : undefined;

  useEffect(() => {
    const fetchToken = async () => {
      const clientSecretBasic = Buffer.from(`${clirenId}:${clirenSecret}`).toString('base64');
      const data = {
      ...
      const body = Object.entries(data)
        .map((entry) => entry[1] ? `${encodeURIComponent(entry[0])}=${encodeURIComponent(entry[1])}` : '')
